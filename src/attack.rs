...
use crate::{weapons::weapons::{Weapon, WeaponDamage}, utils::{dice::{Die, Rollable, is_natural_20, beats_dc}, probability::Meanable}};
...
pub struct Attack<T: Rollable<u16> + Meanable> {
    attack_bonus: i8,
    damage_dice: T,
    damage_bonus: i8,
}

impl <T: Rollable<u16> + Meanable> Attack<T> {
    ...
    pub fn roll_attack(&self, ac: i16) -> HitResult {
        ...
        let is_hit = beats_dc(hit_roll as i16, ac - (self.attack_bonus as i16));
    ...
    pub fn calculate_total_damage(&self, hit_result: HitResult) -> u16 {
        match hit_result {
            HitResult::Miss => 0,
            HitResult::Hit => {
                let total_dice_roll: u16 = self.damage_dice.roll();
...
pub(crate) fn from_weapon_and_stats(weapon: Weapon, stats: &crate::player::StaticPlayerStats) -> Attack<WeaponDamage> {
